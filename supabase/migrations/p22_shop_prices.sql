-- =============================================================
-- p22: PX Store prices — refreshments cheaper than equipment.
-- NGU patch is not sold (RPC rejects new/any buy).
-- Safe to re-run (replaces buy_shop_item only).
-- =============================================================
...
BEGIN
  ...
  -- Hidden from PX Store until the chevron art is ready.
  IF p_item_id = 'patch' THEN
    RETURN jsonb_build_object('error', 'not_purchasable');
  END IF;
  ...
  v_price := CASE p_item_id
    ...
    WHEN 'energy'    THEN 15
    WHEN 'thermos'   THEN 28
    WHEN 'boots'     THEN 110
    WHEN 'kneepads'  THEN 75
    WHEN 'belt'      THEN 10
    WHEN 'vest'      THEN 230
    ...
    WHEN 'watch'     THEN 70
    WHEN 'radio'     THEN 160
    WHEN 'glasses'   THEN 90
    WHEN 'headset'   THEN 190
    WHEN 'helmet'    THEN 280
    ELSE NULL
  END;
